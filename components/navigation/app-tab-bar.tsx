...
const TABS = [
  ...
  { key: 'accounts', label: 'Accounts', href: '/dashboard/accounts', icon: CreditCard },
  { key: 'activity', label: 'Activity', href: '/dashboard/activity', icon: ReceiptText },
  { key: 'stats', label: 'Stats', href: '/dashboard/stats', icon: BarChart3 },
  { key: 'profile', label: 'Profile', href: '/dashboard/settings', icon: UserRound },
] as const
  ...
  return (
    <>
      <aside className="hidden lg:fixed lg:inset-y-0 lg:left-0 lg:z-40 lg:flex lg:w-[248px] lg:flex-col lg:border-r lg:border-[#162019] lg:bg-[#08100c]/85 lg:px-4 lg:py-6 lg:backdrop-blur">
        <Link href="/dashboard" className="flex items-center gap-3 rounded-[22px] px-3 py-2">
          <div className="flex size-11 items-center justify-center rounded-full bg-[#1a2c1f]">
            <span className="text-base font-bold text-[#8bff62]">P</span>
          </div>
          <div>
            <p className="text-sm font-semibold text-[#f4f7f5]">Penni</p>
            <p className="text-xs text-[#7f8c86]">Finance, calmer</p>
          </div>
        </Link>
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 border-t border-[#162019] bg-[#08100c] pb-[env(safe-area-inset-bottom)] pt-2 lg:hidden">
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  isActive
                    ? 'bg-[#8bff62] scale-100 shadow-[0_0_15px_rgba(139,255,98,0.2)]'
                    : 'bg-transparent'
                )}
              >
                <tab.icon
