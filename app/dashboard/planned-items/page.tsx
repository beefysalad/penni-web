...
import { useAccountsQuery, usePlannedItemsQuery } from '@/hooks/use-finance-queries'
import type { CategoryType, PlannedItem, RecurrenceFrequency } from '@/lib/finance.types'
import { formatCurrency } from '@/lib/formatters'
import { Calendar, Plus, Sparkles } from 'lucide-react'

type DraftPlannedItemForm = {
  type: CategoryType
  title: string
  amount: string
  accountId: string
...
const DEFAULT_PLANNED_ITEM_FORM: DraftPlannedItemForm = {
  type: 'EXPENSE',
  title: '',
  amount: '',
  accountId: '',
...
export default function PlannedItemsPage() {
  ...
  const [form, setForm] = useState<DraftPlannedItemForm>(DEFAULT_PLANNED_ITEM_FORM)
  const [draftItems, setDraftItems] = useState<PlannedItem[]>([])

  const accounts = accountsQuery.data ?? []
  const plannedItems = useMemo(
    () => [...draftItems, ...(plannedItemsQuery.data ?? [])],
    [draftItems, plannedItemsQuery.data]
  )

  const incomeItems = useMemo(() => plannedItems.filter((item) => item.type === 'INCOME'), [plannedItems])
  const expenseItems = useMemo(() => plannedItems.filter((item) => item.type === 'EXPENSE'), [plannedItems])
  ...
  const handleCreatePlannedItem = () => {
    ...
    if (requiresAccount && !form.accountId) return

    const timestamp = new Date().toISOString()
    const semiMonthlyDays = isSemiMonthly
      ? [Number(form.firstSemiMonthlyDay), Number(form.secondSemiMonthlyDay)].filter((value) => Number.isFinite(value))
      : []

    setDraftItems((current) => [
      {
        id: `draft-planned-${Date.now()}`,
        ...
        semiMonthlyDays,
        isActive: true,
        nextOccurrenceAt: new Date(form.startDate).toISOString(),
        createdAt: timestamp,
        updatedAt: timestamp,
      },
      ...current,
    ...
    <>
      <DashboardHeaderShell>
        <AppPageHeader
          ...
          title="Planned Items"
          subtitle="Draft recurring bills and income in one place. This pass is frontend-only so we can shape the flow before wiring create mutations."
          inverted
        />
      </DashboardHeaderShell>
      ...
      <div className="flex flex-col gap-6 px-4 pb-28 pt-6 md:px-6 lg:px-8">
        <div className="flex items-center justify-between rounded-[24px] border border-[#17211c] bg-[#111916] px-4 py-3">
          <div>
            <p className="text-[12px] font-bold uppercase tracking-[1.8px] text-[#4a5650]">Planning flow</p>
            <p className="mt-1 text-[14px] font-medium text-[#93a19a]">Use this page to shape recurring bills and salary setup on web.</p>
          </div>
          <FrontendOnlyBadge />
        </div>
        ...
        <div className="grid gap-6 xl:grid-cols-[minmax(0,430px)_minmax(0,1fr)]">
          <div className="space-y-6">
            <div className="rounded-[30px] border border-[#17211c] bg-[#111916] p-5">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-[11px] font-bold uppercase tracking-[2px] text-[#4a5650]">Plan ahead</p>
                  <h2 className="mt-2 text-[24px] font-bold tracking-tight text-[#f4f7f5]">Add a recurring item</h2>
                  <p className="mt-2 text-[14px] font-medium leading-relaxed text-[#7f8c86]">
                    Bills and income can both live here. Income is tied to an account so we keep the flow close to mobile.
                  </p>
                </div>
              ...
              <div className="mt-6 space-y-5">
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      setForm((current) => ({
                        ...
                        accountId: event.target.value === 'EXPENSE' ? current.accountId : current.accountId,
                      }))
                    }
                ...
                <div className="space-y-2">
                  <Label htmlFor="planned-title">{form.type === 'INCOME' ? 'Income name' : 'Bill name'}</Label>
                  <Input
                    id="planned-title"
                    value={form.title}
                    onChange={(event) => setForm((current) => ({ ...current, title: event.target.value }))}
                    placeholder={form.type === 'INCOME' ? 'e.g. Payroll, Freelance retainer' : 'e.g. Rent, Internet, Credit card'}
                  />
                </div>
                ...
                <div className="space-y-2">
                  <Label htmlFor="planned-date">{form.type === 'INCOME' ? 'First payout date' : 'First due date'}</Label>
                  <Input
                    id="planned-date"
                    type="date"
              ...
              <div className="mt-6 flex gap-3">
                <Button onClick={handleCreatePlannedItem} className="flex-1">
            ...
            <CreationHint
              title="Frontend flow first"
              description="This lets us validate the page, form shape, and account-linking experience before the web starts mutating backend planned items for real."
              actionLabel="Backend save comes next"
            />
          </div>
          ...
          <div className="space-y-6">
            <div className="rounded-[30px] border border-[#17211c] bg-[#0f1512] p-5">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <h3 className="text-[26px] font-bold tracking-tight text-[#f4f7f5]">Recurring expenses</h3>
                  <p className="mt-1 text-[14px] font-medium text-[#7f8c86]">Bills, rent, subscriptions, and card dues.</p>
                </div>
              ...
              <div className="mt-5 overflow-hidden rounded-[24px] border border-[#17211c] bg-[#111916]">
                {plannedItemsQuery.isLoading ? (
                  <div className="space-y-3 p-4">
                    <div className="h-16 rounded-[20px] bg-[#131b17] animate-pulse" />
                    <div className="h-16 rounded-[20px] bg-[#131b17] animate-pulse" />
                  </div>
                ) : expenseItems.length > 0 ? (
                  expenseItems.map((item, index) => (
                    <PlannedItemRow key={item.id} item={item} isLast={index === expenseItems.length - 1} />
                  ))
                ) : (
                  <div className="p-4">
                    <FinanceEmptyState
            ...
            <div className="rounded-[30px] border border-[#17211c] bg-[#0f1512] p-5">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <h3 className="text-[26px] font-bold tracking-tight text-[#f4f7f5]">Recurring income</h3>
                  <p className="mt-1 text-[14px] font-medium text-[#7f8c86]">Salary, allowances, and repeat payouts.</p>
                </div>
              ...
              <div className="mt-5 overflow-hidden rounded-[24px] border border-[#17211c] bg-[#111916]">
                {plannedItemsQuery.isLoading ? (
                  ...
                  incomeItems.map((item, index) => (
                    <PlannedItemRow key={item.id} item={item} isLast={index === incomeItems.length - 1} />
                  ))
                ) : (
                  <div className="p-4">
                    <FinanceEmptyState
                      ...
                      description="Add salary or repeat payouts here so Home can eventually project incoming cash more clearly."
                    />
                  </div>
                )}
              </div>
