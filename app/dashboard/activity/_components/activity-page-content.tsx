...
} from '../_lib/activity-page.helpers'
...
export function ActivityPageContent() {
  ...
  const filteredTransactions = useMemo(() => {
    return allTransactions.filter((transaction) => {
      ...
      const matchesType =
        ...
        (activeTypeFilter === 'Expenses' && transaction.type === 'EXPENSE') ||
        (activeTypeFilter === 'Income' && transaction.type === 'INCOME')
      return matchesSearch && matchesType
    })
  }, [activeTypeFilter, allTransactions, searchQuery])
  ...
  }, [accountId, mode, setValue, transferSourceAccounts])
      ...
      createTransferMutation.mutate(
        {
          ...
          transactionAt: toTransactionIsoDate(values.transactionAt),
        },
        {
          onSuccess: () => {
            toast.success(
              isCardPayment ? 'Card payment recorded.' : 'Transfer recorded.'
            )
            reset({
              ...DEFAULT_TRANSACTION_FORM,
              mode: values.mode,
    ...
    >
      ...
      <div className="mt-6 grid gap-4 max-lg:mt-0 xl:grid-cols-2">
        ...
        <div className="hidden space-y-2 lg:block">
          ...
          <select
            ...
            value={mode}
            onChange={(event) => {
              const newMode = event.target.value as TransactionForm['mode']
              const nextAccountId =
                newMode === 'TRANSFER' &&
                !transferSourceAccounts.some((account) => account.id === accountId)
                  ? ''
          ...
          >
            ...
            <option value="TRANSFER">
              {isCardPayment ? 'Payment' : 'Transfer'}
            </option>
          </select>
        </div>
        ...
        <div className="space-y-2">
          <Label htmlFor="transaction-amount">
            {isCardPayment ? 'Payment amount' : 'Amount'}
          </Label>
          <Input
        ...
        {mode === 'TRANSFER' ? (
          <div className="space-y-2">
            <Label htmlFor="transaction-to-account">
              {isCardPayment ? 'Card' : 'To account'}
            </Label>
            <select
              id="transaction-to-account"
              value={toAccountId}
              onChange={(event) =>
                setValue('toAccountId', event.target.value, {
                  shouldDirty: true,
                  shouldTouch: true,
                  shouldValidate: true,
                })
      ...
      <div className="mt-6 flex gap-3">
        ...
        <Button
          ...
          variant="secondary"
          onClick={() => {
            reset(DEFAULT_TRANSACTION_FORM)
            setShowComposer(false)
          }}
        >
          Cancel
          ...
          {transactionsQuery.isLoading ? (
            ...
            sections.map((section) => (
              <div key={section.title} className="space-y-3">
                ...
                <div className="overflow-hidden rounded-[30px] border border-[#17211c] bg-[#111916]">
                    ...
                    <TransactionRow
                      ...
                      accountLabel={
                        transaction.accountId
                          ? accountNameMap.get(transaction.accountId)
                          : null
                      }
                      accountType={
                        transaction.accountId
                          ? accountTypeMap.get(transaction.accountId)
                          : null
                      }
