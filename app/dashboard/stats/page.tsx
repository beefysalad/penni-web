...
import {
  buildMonthlyExpenseDistribution,
  getCurrentMonthBounds,
  getTransactionMonthOptions,
} from '@/lib/selectors'
import { formatCurrency } from '@/lib/formatters'
import { BarChart3, PieChart, Activity, Wallet } from 'lucide-react'
...
export default function StatsPage() {
  ...
  const selectedPeriod = useMemo(() => {
    const selectedOption = monthOptions.find(
      (option) => option.value === effectiveMonth
    )
  ...
  return (
    <>
      ...
      <div className="animate-in fade-in flex flex-col gap-6 px-4 pt-6 pb-28 duration-500 md:px-6 lg:px-8">
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-[14px] font-medium text-[#93a19a]">
                Stats default to this month, but you can switch to an earlier
                month anytime.
              </p>
            </div>
            ...
            <select
              ...
              className="h-12 min-w-[220px] rounded-[1.2rem] border border-[#17211c] bg-[#131b17] px-4 text-[15px] font-medium text-[#f4f7f5] transition outline-none focus:border-[#2a3a31] focus:ring-2 focus:ring-[#2a3a31]/30"
            >
              <option value={currentMonthValue}>
                {new Intl.DateTimeFormat('en-US', {
                  month: 'long',
                  year: 'numeric',
                }).format(new Date(currentPeriod.year, currentPeriod.month, 1))}
              </option>
              {monthOptions
        ...
        {transactionsQuery.isLoading || categoriesQuery.isLoading ? (
          <div className="h-64 w-full animate-pulse rounded-[30px] bg-[#111916]" />
        ) : (
          <div className="flex flex-col items-center rounded-[30px] border border-[#17211c] bg-[#111916] p-8 shadow-xl shadow-black/20">
            <h4 className="mb-6 text-[11px] font-bold tracking-[2px] text-[#4a5650] uppercase">
              Monthly overview
            </h4>
            <ExpenseDonut
            ...
            <div className="mt-10 flex w-full flex-row justify-between border-t border-[#1b2a21]/30 pt-6">
              <div className="flex flex-1 flex-col items-center">
                <p className="text-[10px] font-bold tracking-widest text-[#4a5650] uppercase">
                  Spending
                </p>
                <p className="mt-1 text-[17px] font-bold text-[#f4f7f5]">
                  {formatCurrency(stats.monthExpenses)}
                </p>
              </div>
              <div className="h-10 w-px bg-[#1b2a21]/30" />
              <div className="flex flex-1 flex-col items-center">
                <p className="text-[10px] font-bold tracking-widest text-[#4a5650] uppercase">
                  Income
                </p>
                <p className="mt-1 text-[17px] font-bold text-[#41d6b2]">
        ...
        <div className="flex flex-row gap-3">
          ...
          <StatTile
            label="Budget drift"
            value={
              stats.monthExpenses > stats.monthIncome ? 'Negative' : 'Soon'
            }
            hint={
              stats.monthExpenses > stats.monthIncome
                ? 'Exceeded income'
                : 'Budget tracking comes next'
            }
            icon={PieChart}
            color="#ffc857"
          />
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2.5">
            {transactionsQuery.isLoading || categoriesQuery.isLoading ? (
              ...
              stats.distributionRows.map((row) => (
                ...
                >
                  ...
                  >
                    <BarChart3
                      className="size-5"
                      style={{ color: row.colorHex ?? '#73827a' }}
                    />
                  </div>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-1 flex flex-row items-center gap-2">
                      <div className="h-1.5 max-w-[80px] flex-1 overflow-hidden rounded-full bg-[#1b2a21]/50">
                        <div
                      ...
                      <span className="text-[12px] font-bold text-[#4a5650]">
                        {row.share}%
                      </span>
                    </div>
                  </div>
              ...
              <div className="flex flex-col items-center justify-center rounded-[30px] border border-[#17211c] bg-[#0f1512] px-6 py-16 text-center">
